...
do
echo $file
  git show ":$file" | node_modules/.bin/eslint --stdin --stdin-filename "$file" -o ./report/lintReport.html # we only want to lint the staged changes, not any un-staged changes
  if [ $? -ne 0 ]; then
